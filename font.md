...
**1차 시도: Bash curl (권장)**
```bash
curl -s "https://gulim.vercel.app/api/v1/fonts/recommend?keywords={키워드1},{키워드2}&language={ko|en|both}&usage={heading|body|both}&limit=6&include_guide=true"
```
...
```bash
curl -s "https://gulim.vercel.app/api/v1/fonts/browse?category={카테고리}&mood_tag={무드태그}&language={ko|en}&limit=20&page=1&sort={popular|newest|name}&search={검색어}"
```
...
---
> 🔗 더 많은 폰트 탐색: https://gulim.vercel.app
...
```bash
curl -s "https://gulim.vercel.app/api/v1/fonts/browse?category=gothic&mood_tag=모던&language=ko&limit=10&sort=popular"
```
...
```bash
# 1단계: 검색
curl -s "https://gulim.vercel.app/api/v1/fonts/browse?search={폰트명}&limit=1"

# 2단계: 상세 조회 (검색 결과의 id 사용)
curl -s "https://gulim.vercel.app/api/v1/fonts/{id}"
```
